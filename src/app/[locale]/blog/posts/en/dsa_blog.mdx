...
summary: "Most of us see a question and just start typing out some logic that comes to our mind. Most of the time we don't give importance to which approach is to be followed."
tag: "beginners, data structures"
---
...
![Data structure and algorithm](/images/blogs/dsa.png)
...
In this blog we are going to cover the basics about Data Structures.
## Topics covered
...
e have now found a way to search the book in constant time independant of your total number of books.   
...
![Sample statistic output](/images/blogs/dsa_op.jpg)

**Code**   
<CodeBlock className="my-24" compact codeInstances={[ { code: 'import time\nimport random\nfrom bisect import bisect\n\nl=[x for x in range(10 ** 4)]\nrandom.shuffle(l)\nhashset=set(l)\ns=[random.randint(1,10 ** 4) for x in range(10 ** 6)]\ns1=time.time()\nfor x in s:\n\t_=x in l\ns2=time.time()\nprint("List:",s2-s1)\n\ns1=time.time()\nl.sort()\nfor x in s:\n\tbisect(l,x)\ns2=time.time()\nprint("Binary Search:",s2-s1)\n\ns1=time.time()\nfor x in s:\n\t_=x in hashset\ns2=time.time()\nprint("Hashset:",s2-s1)', label: 'Next.js', language: 'js' }, ]} copyButton />
...
![Classification of data structures](/images/blogs/dsa_types.png)   
...
<Table data={{
  headers: ['Operation', 'Array Implementation', 'Linked List Implementation'],
  rows: [
    ['Search', 'O(n)', 'O(n)'],
    ['Insert', 'O(n)', 'O(1)'],
    ['Delete', 'O(n)', 'O(1)'],
...
![stack image](/images/blogs/stack.png)   
...
![queue image](/images/blogs/queue.png)   
...
![Binary tree vs Binary search tree](/images/blogs/bt_bst.webp)   

#### Time complexity of BST   

<Table data={{
  headers: ['Operation', 'Average Time Complexity'],
  rows: [
...
<br/>Using BST to search for a word of length M in a set of N keys then time time complexity would be `O(M*log N)`[for a well balanced BST], but using Trie we can bring it down to `O(M)`.   
<br/>But at what cost do we get this benefit?   
<br/>The space complexity of a Trie.   
...
![trie tree image](/images/blogs/trie_tree.png)   
<br/>
...
![B tree image](/images/blogs/b-tree.png)   
...
![graph image](/images/blogs/g1.png)  

![end image](/images/blogs/end_dsa.jpg)   
Data structures by itself is a very extensive topic and as stated earlier I have tried my best to keep it short and to the point but data structures have a lot more to offer and everything mentioned above is just a bird's view of the vast world of data structures.

Kudos to those who had the patience to read the entire blog.   
