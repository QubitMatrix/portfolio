...
It is a good practice to pull before making new changes, however if the remote tracking branch has moved ahead after making the changes and the push fails, run `git pull` to get latest updates and `git push`. If it still fails, merge the remote changes by running `git merge --no-ff` and then push again.
...
**Workflow**   
1. **Find hash** - `git log --oneline` to find the commit that is needed 
